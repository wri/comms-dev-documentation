...
We maintain our wildcard cert with DigiCert. 
  * To generate a new cert run the CSR (certificate signing request) on the AWS EC@ 'redirect server' (```$ openssl req -new -newkey rsa:2048 -nodes -keyout wri_cert.key -out wri_cert.csr```)
